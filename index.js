...
function makeScatterPlot(csvData) {
    ...
    let mapFunctions = drawAxes(axesLimits, "fertility", "life_expectancy", svgContainer, m);

    // plot data as points and add tooltip functionality
    plotData(mapFunctions);

    // draw title and axes labels
    makeLabels(svgContainer, m, "Countries by Life Expectancy and Fertility Rate",'Fertility Rates (Avg Children per Woman)','Life Expectancy (years)');
...
function makeLabels(svgContainer, m, title, x, y) {
    svgContainer.append('text')
        .attr('x', (m.width - 2 * m.marginAll) / 2 - 90)
        .attr('y', m.marginAll / 2 + 10)
        .style('font-size', '10pt')
        .text(title);

    svgContainer.append('text')
        .attr('x', (m.width - 2 * m.marginAll) / 2 - 30)
        .attr('y', m.height - 10)
        .style('font-size', '10pt')
        .text(x);

    svgContainer.append('text')
        .attr('transform', 'translate( 15,' + (m.height / 2 + 30) + ') rotate(-90)')
        .style('font-size', '10pt')
        .text(y);
}
...
function plotData(map) {
    ...
    let toolChart = div.append('svg')
        .attr('width', mini_m.width + 100)
        .attr('height', mini_m.height + 100)
...
function plotPopulation(country, toolChart) {
    let country_data = data.filter((row) => {return row.country == country})
    let population = country_data.map((row) => parseInt(row["population"]));
    let year = country_data.map((row) => parseInt(row["year"]));

    let axesLimits = findMinMax(year, population);
    let mapFunctions = drawAxes(axesLimits, "year", "population", toolChart, mini_m);
    ...
    makeLabels(toolChart, mini_m, "Population Over Time For " + country, "Year", "Population (in Millions)");
}

// draw the axes and ticks
function drawAxes(limits, x, y, svgContainer, m) {
    ...
    let xScale = d3.scaleLinear()
        ...
        .range([0 + m.marginAll, m.width - m.marginAll])
    ...
    svgContainer.append("g")
        .attr('transform', 'translate(0, ' + (m.height - m.marginAll) + ')')
        .call(xAxis);
    ...
    let yScale = d3.scaleLinear()
        ...
        .range([0 + m.marginAll, m.height - m.marginAll])
    ...
    svgContainer.append('g')
        .attr('transform', 'translate(' + m.marginAll + ', 0)')
        .call(yAxis);
